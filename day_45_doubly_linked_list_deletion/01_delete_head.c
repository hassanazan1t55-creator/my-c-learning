...
struct Node* deleteFirstNode(struct Node *head) {
    ...
    struct Node *temp = head; // 1. Keep track of the old head to free it later
    head = head->next;        // 2. Move head to the next node
    
    if (head != NULL) {
        head->prev = NULL;    // 3. Set the prev pointer of the new head to NULL
    }
    
    free(temp);               // 4. Free the old head node from memory
