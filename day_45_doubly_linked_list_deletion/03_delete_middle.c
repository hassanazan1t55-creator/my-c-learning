...
void deleteMiddleNode(struct Node *head, int position) {
    ...
    // 1. Move to the node that we want to delete
    ...
    // 2. Link the previous node's next pointer to the next node
    if (temp->prev != NULL) {
        temp->prev->next = temp->next;
    }
    
    // 3. Link the next node's prev pointer to the previous node
    if (temp->next != NULL) {
        temp->next->prev = temp->prev;
    }
    
    free(temp); // 4. Free the target node from memory
}
