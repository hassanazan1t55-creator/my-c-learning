...
// 1. Normal Structure with zero optimization (Huge memory waste)
struct NormalSensor {
  char sensor_type;  // 1 byte
  double reading;    // 8 bytes (Most heavy data type)
  int status_code;   // 4 bytes
};

// 2. Fully Packed Structure (Zero memory waste, exact sizes)
#pragma pack(push, 1)
struct PackedSensor {
  char sensor_type;  // 1 byte
